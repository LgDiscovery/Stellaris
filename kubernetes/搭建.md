...
kubeadm join 192.168.56.101:6443 --token kps5e6.58h3vq6kb6srm9tb \
        --discovery-token-ca-cert-hash sha256:545eb7849bbfa8450854c81e2412fb02be67960b23d52e67f39b17ee688b82eb -- 之前的

kubeadm join 192.168.56.101:6443 --token tj16s1.w1kykztybl3l0qqm --discovery-token-ca-cert-hash sha256:60c394752b1a683d817e02cc94965f23f93af2bc71a5e30430790fef700727a1



# 1. 先删除旧token（可选，确保环境干净）
kubeadm token delete 9rcvhn.gw9igk0cuk2h5kcq 2>/dev/null || echo "旧token已不存在，跳过"

# 2. 生成新的join命令（直接复制输出结果）
...
步骤 1：在能联网的机器上拉取并导出`nginx`镜像

找一台可以正常访问互联网的机器（非集群节点），执行：
...
通过`scp`把`nginx.tar`传到`master、node1、node2`：

```bash
# 传到master
scp nginx.tar root@k8s-master:/opt/
# 传到node1
scp nginx.tar root@k8s-node1:/opt/
# 传到node2
scp nginx.tar root@k8s-node2:/opt/
```

步骤 3：在集群节点导入镜像到`containerd`

在`master、node1、node2`分别执行：

```bash
# 导入镜像到containerd（k8s.io命名空间）
ctr -n k8s.io images import /opt/nginx.tar
```
...
# 创建新的Deployment（直接使用本地导入的nginx镜像）
kubectl create deployment nginx --image=nginx
# 查看Pod状态（等待READY变为1/1）
kubectl get pod
```

containerd 对应的登录工具是ctr（containerd 的原生命令行客户端），针对你的 HTTP 协议 Harbor，登录步骤如下：

1. 执行登录命令
   直接使用ctr login，并添加--plain-http参数（适配 HTTP 协议的 Harbor）：
   bash
   运行

# 替换为你的Harbor密码

ctr login --user admin --password 你的Harbor密码 --plain-http 192.168.56.103:8085
2. 验证登录状态
登录成功后，可通过以下命令查看已信任的仓库凭据：
bash
运行

# 查看containerd的登录凭据

cat /var/lib/containerd/io.containerd.grpc.v1.cri/configs/192.168.56.103:8085/auth.json
3. 拉取 Harbor 镜像（登录后操作）
登录成功后，即可拉取 Harbor 中的镜像：
bash
运行

# 替换为你的Harbor镜像路径

ctr images pull --plain-http 192.168.56.103:8085/项目名/镜像名:标签
（若需用 K8s 的crictl工具拉取，直接执行crictl pull 192.168.56.103:8085/项目名/镜像名:标签即可，它会复用 containerd 的登录凭据）
